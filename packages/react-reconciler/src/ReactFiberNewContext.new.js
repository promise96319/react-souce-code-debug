...
// context 改变了，递归寻找 使用了 context 的组件
// 如果是 class 组件，那么会添加一个 update，标识为 forceUpdate
...
export function checkIfContextChanged(currentDependencies: Dependencies) {
  ...
  while (dependency !== null) {
    ...
    // 对比 memoizedValue 和 context.value
...
// 判断 dependencies 是否存在，如果存在 firstContext，说明有更新
// 将 didReceiveUpdate 置为 true，并将 firstContext 置为 null
export function prepareToReadContext(
  workInProgress: Fiber,
  renderLanes: Lanes,
): void {
  // 记录 当前 fiber 为 currentlyRenderingFiber
  // 在 hooks 使用的时候会用到
  currentlyRenderingFiber = workInProgress;
  lastContextDependency = null;
  lastFullyObservedContext = null;

  // todo content 的 consumer / provider ?
...
// 读取 context 的 value，并且在 fiber 上创建 dependencies，形成链表结构
