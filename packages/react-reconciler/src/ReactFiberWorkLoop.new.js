...
// 获取当前时间
// 正在执行 react render/commit 则返回当前时间
// 否则返回 之前记录的时间
export function requestEventTime() {
  // 在 react 的 render 或 commit 阶段，直接访问现在的时间
  if ((executionContext & (RenderContext | CommitContext)) !== NoContext) {
    // We're inside React, so it's fine to read the actual time.
    return now();
  }

  // 在其他阶段时，如果有起始时间，返回起始时间
...
// 返回 fiber 对应的 lane
// concurrentMode 下。render => transition => update => event
export function requestUpdateLane(fiber: Fiber): Lane {
  // Special cases
  const mode = fiber.mode;
  // concurrentMode 模式下包含 concurrentMode
  // createRoot 模式下是 concurrentMode
  // 即 rootFiber 返回的是 SyncLane
  // 不是 ConcurrentMode 那么不会并发执行，也就是全部为同步任务。
  if ((mode & ConcurrentMode) === NoMode) {
    ...
    // 如果正处在 render 阶段，获取 render 的最高优先级
    // todo 什么时候在 render 阶段
...
export function scheduleUpdateOnFiber(
  ...
  // 更新 fiber root 上 pendingLanes，加入了 lane
  // 计算lane所在位置，并将 eventTime 放到 eventTimes 里
  ...
  // workInProgressRoot 存在并且和 root 相同
  // todo 这里的处理不是很明白
  if (root === workInProgressRoot) {
    ...
    // todo 向后兼容，render 阶段不将其处理为 interleaved
    if (
      deferRenderPhaseUpdateToNextBatch ||
      (executionContext & RenderContext) === NoContext
    ) {
      // 如果不处于 render 阶段，合并 lane
...
// exiting a task.
// 为 root 调度一个任务。
// 每个 root 只能有一个任务。如果任务已经存在，那么会检查对比优先级，并确保高优先级先执行。
// 该函数在每次更新或者退出一个任务之前执行。
// 最终会走到 scheduleSyncCallback/scheduleCallback 进行执行 performUnitWork
function ensureRootIsScheduled(root: FiberRoot, currentTime: number) {
  // root.callbackNode 是在每次调度完赋值的，也就是本函数的末尾
  const existingCallbackNode = root.callbackNode;

  // Check if any lanes are being starved by other work. If so, mark them as
  // expired so we know to work on those next.
  // 递归 pendingLanes，如果 lane 过期了会加入到 expiredLanes 中
  // 如果任务一直被打断，那么时间到了就会将其置为过期。
  ...
  // 获取 nextLanes 中的最高优先级，返回对应的 lane 数字
  ...
  // 如果与当前优先级没有变化，那么直接返回。
  // 这就是为什么能实现批量更新的原理
  // 首先 setState 会进行 schedule，再下一次 setState 的时候，由于优先级相同，不会 schedule
  if (
...
// didTimeout 是在 Scheduler 的 workLoop 中执行任务时
// 返回的当前任务的时间是否过期（但是一定是超过 startTime 的）
function performConcurrentWorkOnRoot(root, didTimeout) {
  ...
  if (didFlushPassiveEffects) {
    ...
    // todo 如果 effect 里的任务优先级更高，取消了当前任务，那么应该开启下一个任务。
  ...
  if (exitStatus !== RootIncomplete) {
    ...
    // exitStatus => completed | suspense
    // 结束了：进行 commit 阶段
    finishConcurrentRender(root, exitStatus, lanes);
  }

  // 完成后，继续 schedule，发现没有 lane 就退出了
  // todo 相当于判断这里有没有更高优先级的任务？
  // 如果没有
...
// 将 root 和 workInProgress 等参数进行初始化
...
function renderRootSync(root: FiberRoot, lanes: Lanes) {
  ...
  if (workInProgressRoot !== root || workInProgressRootRenderLanes !== lanes) {
    ...
    // 开始准备工作，将 workInProgress 等参数进行初始化。
    // 包括准备 workInProgress
...
function renderRootConcurrent(root: FiberRoot, lanes: Lanes) {
  ...
  // If the root or lanes have changed, throw out the existing stack
  // and prepare a fresh one. Otherwise we'll continue where we left off.
  // todo: 如果 root 或者 lanes 变化？什么时候变化了？变化了做什么处理
  if (workInProgressRoot !== root || workInProgressRootRenderLanes !== lanes) {
    if (enableUpdaterTracking) {
      if (isDevToolsPresent) {
        const memoizedUpdaters = root.memoizedUpdaters;
...
// 如果节点已经通过 beginWork了，进行 completeWork 创建真实节点。
// 如果节点 Incomplete，如果是 Suspense ，标记 DidCapture => 继续 com
  ...
  do {
    ...
    if ((completedWork.flags & Incomplete) === NoFlags) {
      ...
      if (
        ...
        // 主要递归更改  childLanes 、 subtreeFlags
        // 如果是 HostComponent 还会创建真实节点，然后放到 fiber.stateNode 中。
      ...
      // 如果有子节点 throw error 了，那么该节点的 flags 就是 Incomplete
      // 将 fiber 的 flags =》 DidCapture
      // 除了 HostRoot 和 Suspense DidCapture 其余的 next 全为 null
      ...
      if (next !== null) {
        ...
        // from the effect tag.
        // 除了 HostRoot 和 Suspense DidCapture 其余的 next 全为 null
        // 标记了所有的 effect，包括 DidCapture
        next.flags &= HostEffectMask;
        // 退出 while 循环，此时会进行 beginWork。
        workInProgress = next;
        return;
      }
      ...
      // 从错误节点到 suspense 之间都是 Incomplete
      // 这里只有 父级元素才会这样
      if (returnFiber !== null) {
    ...
    // 如果他有兄弟元素，那么退出循环，然后开始兄弟元素的 beginWork
    if (siblingFiber !== null) {
      // If there is more work to do in this returnFiber, do that next.
      workInProgress = siblingFiber;
      return;
    }
    // 如果他没有了兄弟元素，此时会找到它的父元素，继续父元素的completeWork
    // 然后是父元素的 sibling 进行 completeWork
    // 以此下去，那么整棵树就创建完成了
...
function commitRootImpl(root, renderPriorityLevel) {
  // todo 这里具体是做什么处理的？
  // 如果是 函数组件，且内部调用了 useEffect，就会在 fiber上增加 passive effects
  // 或者是 有节点删除时，找到 function 组件，触发 destroy + create
  // todo rootWithPendingPassiveEffects 存在时，就会依次执行 unMount 吗？
  do {
  ...
  if (
    ...
    if (!rootDoesHavePassiveEffects) {
      ...
      // 如果使用了 useEffect => Passive 存在 => schedule
      // 按 normal 优先级 调度 flushPassiveEffects
      // todo 为什么要 schedule
  ...
  if (subtreeHasEffects || rootHasEffect) {
    ...
    // beforeMutation 阶段
    // 递归调用 getSnapshotBeforeUpdate 函数，获取先前的 props, state
  ...
  if (rootDoesHavePassiveEffects) {
    ...
    // 记录 记录pendingPassiveEffects,在下一次更新的时候需要使用到。
    rootWithPendingPassiveEffects = root;
    // 记录 pendingPassiveEffectsLanes,在下一次更新的时候需要使用到。
...
// 如果存在 effects，循环调用 unmount 和 mount 
// 如 useEffect/useLayoutEffect里的函数
...
// 如果 存在 rootWithPendingPassiveEffects
// 执行 effect 的 unmount 和 mount
function flushPassiveEffectsImpl() {
  if (rootWithPendingPassiveEffects === null) {
    return false;
  }

  // 清空上次使用的 lanes effects
  ...
  // 执行 unmount 和 mount effect
  // root.current 还是老的 current
  ...
  // 执行同步回调，
  // todo 为什么这个位置需要执行？
  // 除非是按照同步的方式进行更新，才会在 syncQueue 中添加渲染任务。
  // todo commit 阶段是同步执行的，会不会是 mount 的时候又设置了新的更新
  // todo 此时同步队列里存在更新，需要直接更新。
  flushSyncCallbacks();
