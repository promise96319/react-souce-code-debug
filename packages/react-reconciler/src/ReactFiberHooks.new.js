...
export function renderWithHooks<Props, SecondArg>(
  ...
  // 函数执行，返回相应的 node，此时遇到了 hooks，会执行 hooks
  // 第二个参数用于 ref转发，例如 forwardRef((props, ref) => ...)
  let children = Component(props, secondArg);

  // Check if there was a render phase update
  // 执行 Component()的时候又进行 setState 的时候会进行 rerender 从而报错
...
// 创建一个 hook, 在函数 fiber 上添加hooks，并返回当前对的hook
...
// 根据 next 获取的对应的 hook。current 和 workInProgress 对比
// 这里主要的目的是为了复用之前的 hook
function updateWorkInProgressHook(): Hook {
  ...
  // 拿到存放在 fiber.memoizedState 上的 hook 链表
  // nextHook 表示当前执行的函数所对应的 hook
  // 这个 fiber 是 current
...
//  会创建一个 effect 链表，存放到 updateQueue 上。
// 且 effect 的 tag 为 PassiveEffect | PassiveStaticEffect
...
function updateEffectImpl(fiberFlags, hookFlags, create, deps): void {
  ...
  // 如果前后依赖有变化，那么就会添加 HookHasEffect
  hook.memoizedState = pushEffect(
    HookHasEffect | hookFlags,
    create,
    destroy,
...
function startTransition(setPending, callback) {
  ...
  ReactCurrentBatchConfig.transition = 1;
  try {
    // 此时进行 schedule 的时候，requestUpdateLane 为 transition lane
    // 添加一个 update
    setPending(false);
    // 添加一个 update
    callback();
...
// 用户点击的时候触发的是这里的函数
// fiber 为 workInProgress => 挂载完成后就是 current了
// 主要将本次更新加入到 hook 的 queue 中
// 如果不是在 render 阶段设置的，那么会进行 scheduler。
// todo 在哪个时间段去除
function dispatchAction<S, A>(
  ...
  // 如果正在执行这个 component 的时候，进行了 dispatch
  // 此时只是将 update 形成链表，等待处理。此时并不会进行 scheduler
  if (
