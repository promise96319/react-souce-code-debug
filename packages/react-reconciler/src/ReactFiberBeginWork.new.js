...
// 如果 workInProgress 有 ref，打上 ref标记
...
function updateFunctionComponent(
  ...
  let nextChildren;
  // 处理 dependencies，如果 firstContext存在，didReceiveUpdate 为 true
  prepareToReadContext(workInProgress, renderLanes);
  if (enableSchedulingProfiler) {
    markComponentRenderStarted(workInProgress);
  }
...
function updateClassComponent(
  ...
  if (instance === null) {
    ...
    // 创建实例
    // 在这个位置上挂载了 updater 里面包含了 setState / forceUpdate等方法
...
// class component进行 render 并 reconciler
...
function updateHostRoot(current, workInProgress, renderLanes) {
  ...
  // 这里的 element 他是 updateContainer 函数里创建 update 传过来的 payload
  // 它表示了 render 渲染时的 element 节点，如 <App />
...
// 创建了一个不知道是 function 还是 class 的 component
// 现在要根据这个 element 创建对应的 fiber
// createElement 的时候，会判断 type 是否是 function 类型
// 如果是 function 且为 class 类型，则是ClassComponent
// 否则是 IndeterminateComponent
function mountIndeterminateComponent(
  ...
  if (
    ...
    // 如果是对象且含有 render，当做 classComponent 处理
    !disableModulePatternComponents &&
    typeof value === 'object' &&
    value !== null &&
    typeof value.render === 'function' &&
    ...
    // 添加 updater
    // 将 instance 存在 fiber.stateNode 上
    // 将 fiber 存在 instance._reactInternals 上
...
// 更新 suspense
// todo suspense 的处理等到 completedWork 的时候再回来看
function updateSuspenseComponent(current, workInProgress, renderLanes) {
  ...
  // 为一个 number，表示不同的 context
  let suspenseContext: SuspenseContext = suspenseStackCursor.current;

  let showFallback = false;
  // DidCapture 是在 unwindWork 里被赋值的
  // 就是捕捉到内部错误时，会进行 completeWork，然后到达 suspense时，
  // 赋值 DidCapture 并重新对 suspense 进行 beginWork
  ...
  // a stack.
  // suspense 像一个错误边界，用于捕捉错误，然后渲染失败，那么重新渲染并且切换到另一颗树。
  // 第一次创建时 current 是不存在的。先显示 primary 捕捉到错误后显示 fallback。
  if (current === null) {
    // Initial mount
    ...
    if (showFallback) {
      ...
      // 如果显示了 fallback ，那么 memoizedState 存在
    ...
    // a fallback.
    // 这是一个更新，因为第一次的时候 current 是不存在。
    const prevState: null | SuspenseState = current.memoizedState;
    // state存在，说明这里已经是显示了 fallback
    if (prevState !== null) {
      ...
      // 不存在先前的 state ，说明之前就已经加载完成了 primary children
      ...
      if (showFallback) {
        ...
        // 渲染 primary children 时，memoizedState置为 null
        workInProgress.memoizedState = null;
        return primaryChildFragment;
      }
    }
...
function mountSuspenseFallbackChildren(
  ...
  // primary 与 fallback fragment 创建联系
  primaryChildFragment.return = workInProgress;
...
function findLastContentRow(firstChild: null | Fiber): null | Fiber {
  ...
  while (row !== null) {
    ...
    // alternate 存在，说明创建了新 fiber => 普通的节点
    // 且深度遍历找不到 suspense
    // 在首次reconciler suspenseList 的时候只有一层 fiber
  ...
  if ((workInProgress.mode & ConcurrentMode) === NoMode) {
    ...
    switch (revealOrder) {
      ...
      case 'backwards': {
        ...
        workInProgress.child = null;
        // 找到第一个 suspense 前面的节点
        // 并将前面的节点倒序
        // tail 指向倒序的第一个元素，row指向第一个 suspense 前面的节点
        while (row !== null) {
          const currentRow = row.alternate;
          // New rows can't be content rows.
          if (currentRow !== null && findFirstSuspended(currentRow) === null) {
...
function updateContextConsumer(
  ...
  const newProps = workInProgress.pendingProps;
  // consumer children 是一个 render 函数
  ...
  // 通过 context 判断是否 参数变化 =》 didReceiveUpdate
  prepareToReadContext(workInProgress, renderLanes);
  // 读取 context 的 value 值
...
// 如果节点前后没有发生变化，提前结束reconcile
// 并且如果子节点还有工作未完成，就返回子节点继续。
// 如果没有更新，可以直接复制下一层的 fiber(这里也有复用)
// 这样就可以避免 reconcile 下一层的 fiber
function bailoutOnAlreadyFinishedWork(
  ...
  // 比如：如果一个 parent component 里 的 childComponent 更新了
  // 那么 parent 不需要更新，可以进一步找 childComponent 对 childComponent 更新
  ...
  // fibers and continue.
  // 如果子节点还有更新，那么直接克隆一份fiber，从子节点继续。
  // 这里相当于跳过了 reconcilerChildren
...
function beginWork(
  ...
  if (current !== null) {
    ...
    if (
      ...
      // props 相同
      // 当前的 lanes 是否包含 renderLanes
      const hasScheduledUpdateOrContext = checkScheduledUpdateOrContext(
        current,
        renderLanes,
      );
      // renderLane 没有这个 lane，说明没有更新，提前结束。
      // 通常只有被 更新的 fiber 他的 lanes 才会改变，其父亲只会改变 childLanes
      if (
    ...
    // 首次 mount，didReceiveUpdate 为 false
    didReceiveUpdate = false;
  }
  ...
  switch (workInProgress.tag) {
    // 第一次创建时，element type 为 function 的组件，tag都是这个
    ...
    // suspense: 处理 fallback 和 primary children
    // todo 里面的 context 的关系是怎样的？
    case SuspenseComponent:
      return updateSuspenseComponent(current, workInProgress, renderLanes);
    case HostPortal:
